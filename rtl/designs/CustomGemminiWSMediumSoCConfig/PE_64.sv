...
module PE_64(
  ...
  always @(posedge clock) begin
    ...
    if (io_in_valid & ~io_in_control_propagate)
  ...
  */
  ...
  MacUnit mac_unit (	// @[PE.scala:64:24]
    .io_in_a  (io_in_a),
    .io_in_b  (io_in_control_propagate ? c2 : c1), // Note: when i preload (WS), io_in_control_propagate=0, so the weights are stored in c2.
                                                  //  When we compute, then, io_in_control_propagate=1, meaning this MAC's input are taken from c2 (preloaded)
    .io_in_c  (io_in_b),  // this is the BIAS input. note that this is propagated thrugh io_in_b (not in_d !!!)
    .io_out_d (io_out_b)  // the MAC's output is propagated as the in_b input to the PEs below through the io_out_b output channel
  );
