...
def extract_tile(tensor, row, col, tile_dim): 
    return aot_operations.extract_tile(tensor, row, col, tensor.shape[0], tensor.shape[1], tile_dim, defs.DType_kInt32).to(conf.GEMM_INPUT_DTYPE) # tensor.dtype


def extract_fp_tile(tensor, row, col, tile_dim): 
    return aot_operations.extract_tile(tensor, row, col, tensor.shape[0], tensor.shape[1], tile_dim, defs.DType_kFloat32)
